...
DECLARE
    ...
    entity text := 'cust';    
    attrbt_nm_metadata_array TEXT[] := ARRAY['metadata.gndr',  'metadata.dyofwk']; -- same length with attrbt_type_array
    attrbt_type_array text[] := array['gndr', 'dyofwk'];
    --
...
BEGIN
    ...
    FOR m IN 1..array_length(agg_func, 1) LOOP
		...
		ELSE
		    ...
		    --  count number of unique customer
